...
export async function upsertMonthlyUsage(agentId: string, inc: UsageIncrements): Promise<void> {
  ...
  await prisma.$executeRaw`
    INSERT INTO agent_usage_monthly (
      ...
      total_summary_tokens, total_deepgram_sec,
      total_recording_sec, whatsapp_msg_count
    ) VALUES (
      ...
      ${inc.totalSummaryTokens ?? 0}, ${inc.totalDeepgramSec ?? 0},
      ${inc.totalRecordingSec ?? 0}, ${inc.whatsappMsgCount ?? 0}
    )
