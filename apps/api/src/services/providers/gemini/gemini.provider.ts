...
export class GeminiProvider implements VoiceProvider {
  ...
  private async openConnection(isReconnect: boolean): Promise<void> {
    ...
    this.connection = new GeminiConnection(url, {
      onSetupComplete: () => {
        ...
        if (this.reconnectAttempts > 0) log.info('Reconnect ready', { attempt: this.reconnectAttempts });
  ...
  private async attemptReconnect(): Promise<void> {
    ...
    try {
      ...
      for (let data of chunks) {
        ...
        // All inbound audio pushed to reconnect buffer is decoded to 8000Hz PCM16
  ...
  private handleServerContent(content: GeminiServerContent): void {
    ...
    if (content.turnComplete) {
      this.state.flushOutputTranscript(null); // passing null prevents emitting to UI
